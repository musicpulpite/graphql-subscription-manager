...
export default class GraphQLSubscriptionManager<PayloadType, ResultType> {
  ...
  public getBatchedAsyncIteratorForSubscription(
    ...
    if (typeof batchInterval !== 'number' || batchInterval <= 0) {
      throw new Error(
        'Cannot generate batched async iterator for a SubscriptionManager with an undefined batch interval.\n Instantiate your SubscriptionManager with an options.batchInterval value or add it as a trailing argument'
      );
    }
